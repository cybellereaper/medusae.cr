...
public final class DiscordFrameworkBinder {
    ...
    private static void registerDefinition(DiscordClient client, DiscordCommandDispatcher dispatcher, CommandDefinition definition) {
        switch (definition.type()) {
            ...
            case USER_CONTEXT ->
                    client.onUserContextMenuContext(definition.name(), context -> dispatcher.dispatch(context.raw(), context));
            case MESSAGE_CONTEXT ->
                    client.onMessageContextMenuContext(definition.name(), context -> dispatcher.dispatch(context.raw(), context));
        }
    }
